    ...
    <div className="absolute left-0 top-0 z-40 h-screen w-screen opacity-100" onClick={() => unloadModal(navigate)}>
      <div id="modal" className="load-slide-fast absolute inset-y-0 left-0 right-0 top-0 z-50 flex h-screen w-3/4 flex-wrap content-center justify-center bg-white opacity-90 duration-75" onClick={(e) => e.stopPropagation()}>
        <div className="h-2/4 w-2/4">
          ...
          <p className="text-3x1 pt-2">Team: Lena Kastell, Matt Oyales, Kimberly Tsang, Jackey Yang, and Issac Zheng</p>
          <p className="text-3x1 pt-4">Ever feel like you just can't make a plan? Feel like you have no idea what to do? We are here to help with that! 
            ...
            nearby locations that match the user's criteria. After that, select the location(s) you would like to go to and we will provide the routes to reach your destination(s)!</p>
          <p className="text-3x1 pt-6">Plannr is a site that will be powered by Google Maps, providing our users with a way to drop a pin on the map to a desired location 
              ...
              list of all available locations that you can select based on your query.</p>
          <p className="text-3x1 text-blue-500 pt-14">Want to get started? Sign up by creating an account with us, or continue as Guest</p>
        </div>
      </div>
    </div>
  );
